...
Теперь в папке `actions` создаеём файл `fire.js` (название действия зависит от названия файла). Теперь мы можем использовать это действие в параметре `actions` при настройке таймеров:
```yaml
actions:
  - '[fire]'
```
...
Мы хотим сделать так, чтобы все игроки в мире поджигались на определённое количество тиков, которое мы сможем указывать в аргументе. При выполнении кастомного действия в движок **_Nashorn_** передаются следующие данные:
- `arg` - Аргумент
- `location` - Объект класса Location, который содержит местоположение таймера (левый нижний угол)

Мы хотим проверить, указан ли аргумент, и если указан, то конвертировать строку аргумента в целое число и передать его в метод _**setFireTicks**_, который вызывается для всех игроков, находящихся в одном мире с таймером
...
Теперь мы можем настроить таймер так, чтобы по окончании времени, всех игроков в мире поджигало на 5 секунд:
```yaml
actions:
  - '[fire] 100'
```

Таким образом можно создавать абсолютно любые действия, имея даже небольшие знания **_JS_** и _**Java**!_

Статьи, которые могут помочь вам в использовании _**Nashorn**_:
